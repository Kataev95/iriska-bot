...
def help_text(
    ...
    rules = [
        ...
        + (f" от {min_len} символов" if min_len > 1 else " любой длины"),
    ]
    if cooldown > 0:
        rules.append(f"• не чаще одного зачёта в {cooldown:g} сек")
    rules.append(
        "• пересланные сообщения, стикеры, фото без текста и команды не считаются"
    )
    if dedupe:
        rules.append("• повтор одного и того же сообщения подряд не считается")
    return (
        ...
        "кто заберёт банк.\n"
        "🧠 Викторина: если бот задал вопрос — первый правильный ответ "
        "в чате забирает приз!"
    )
